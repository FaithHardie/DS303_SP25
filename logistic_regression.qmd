...
This data is showing credit card default. The aim is to predict which customers will default on their credit card debt. It is logistic in it is all no or yes. wether someone defaults or is a student is binary.
...
```{r}
# use ggplot
#ggplot(Default, aes(x = income, y = balance)) +
#  geom_point() 
##no relationship
```
...
The log odds of someone defaulting when income is 0.

**Income:**

As income increases the log odds of someone defaulting will decrease.
...
```{r}
default_practice <- glm(default ~ income + student + balance, data = default, family = binomial)
#summary(default_practice)
default_0 <- glm(default ~ income + balance, data = default, family = binomial)
summary(default_0)
```

Interpret your findings here:

When predicting if someone will default on their credit card, the most significant predictors are income and their credit card balance combined.

Someone with a higher credit card balance combined with a lower income is the most likely predictor of defaulting on their credit card balance.
