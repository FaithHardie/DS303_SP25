...
```{r}
m_simple <- lm(price ~ carat, data = train_data)
m_complex <- lm(price ~ carat+ cut + color + depth, data = train_data)
```

## Step 3: Predict on the Test Set

```{r}
pred_simple <- predict(m_simple, test_data)
pred_complex <- predict(m_complex, test_data)
...
rmse(train_data$price, pred_simple)
rmse(train_data$price, pred_complex)
```

Which is the better model and why?

The complex model is better because it has a higher RMSE score, it more accurately predicts the price of a diamond.
