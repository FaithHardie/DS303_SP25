...
The data for the class wide dataset, DS303_combined.csv

In addition to Dr. Camp removing mispronounced words, we converted certain variables to factors and removed the words "cot" and "caught." The variables changed to factors were
...
ggplot(class_clean, aes(x= f1, y=f2, fill=ethnicity))+
  geom_point()
#par()
#with(class_clean, plotVowels(f1, f2, var.sty.by = sex, var.col.by = ethnicity, pretty = TRUE))
```
