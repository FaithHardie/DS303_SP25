...
The purpose of this project is to analyze the different components of how our class says words, including the vowel, onset, offset, and environmental factors that can affect how we speak. My variables of interest are real word, f0, onset and offset. I'm interested in if a persons pitch changes if the word is real or not, and how they pronounce real words vs. non real words.
...
```{r}
#ggplot(class_clean, aes(x = arpa , y = f2)) +
#  geom_boxplot()
#ggplot(class_clean, aes(x= f1, y=f2, fill=ethnicity))+
#  geom_point()
par()
with(class_clean, plotVowels(f1, f2, var.sty.by=sex, var.col.by = offset, pretty = TRUE,  legend.kwd = "bottomleft"))
```
